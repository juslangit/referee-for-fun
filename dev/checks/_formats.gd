...
## Do the two sports that have both formats actually play them differently?
##
...
## assumed, in both sports, both ways round.
...
func _check(name: String, scene: String, sport: StringName, doubles: bool) -> void:
	...
	print("%-10s %-9s %8d %10.2f %14.2f" % [
		name, "doubles" if arena.playing_doubles() else "singles",
