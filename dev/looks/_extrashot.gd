...
func _ready() -> void:
	...
	get_viewport().get_texture().get_image().save_png("res://dev/shots/_shot_extras.png")
	# And the line camera's own picture, at the size the player actually sees it.
	arena.shuttle_cam.texture().get_image().save_png("res://dev/shots/_shot_shuttlecam.png")
